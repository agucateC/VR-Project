...
// Modifica tu función updateHealthBar
function updateHealthBar() {
  const healthPercent = Math.max(playerHealth / 5, 0);
  
  if (renderer.xr.isPresenting) {
    // Modo VR - usa el HUD 3D
    vrHealthHUD.update(healthPercent);
    document.getElementById('hud').style.display = 'none'; // Oculta HUD HTML
  } else {
    // Modo normal - usa HUD HTML
    document.getElementById('hud').style.display = 'block'; // Muestra HUD HTML
...
function throwBall() {
  ...
  sphere.collider.center.copy(camera.position).addScaledVector(playerDirection, playerCollider.radius * 1.5);

  const impulse = 25; // Fuerza constante en VR
...
function animate() {
  // Usar el bucle de animación de WebXR si está activo
  if (renderer.xr.isPresenting) {
